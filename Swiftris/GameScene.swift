...
class GameScene: SKScene {
    ...
    func addPreviewShapeToScene(shape:Shape, completion:() -> ()) {
        for (idx, block) in enumerate (shape.blocks) {
            ...
            let moveAction = SKAction.moveTo(pointForCol(block.column, row: block.row), duration: NSTimeInterval(0.2))
    ...
    func BOOM(linesToRemove: Array<Array<Block>>, fallenBlocks: Array<Array<Block>>, completion: () -> ()) {
        var longestDuration: NSTimeInterval = 0
        
        for (colID, col) in enumerate(fallenBlocks) {
            for (blockID, block) in enumerate (col) {
                let newPosition = pointForCol(block.column, row: block.row)
                let sprite = block.sprite!
                let delay = (NSTimeInterval(colID) * 0.05) + (NSTimeInterval(blockID) * 0.05)
        ...
        for (rowID, row) in enumerate(linesToRemove) {
